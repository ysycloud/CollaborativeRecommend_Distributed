...
public class Step4_Update {
    ...
    public static class Step4_PartialMultiplyMapper extends Mapper<LongWritable, Text, Text, Text> {
        ...
        public void map(LongWritable key, Text values, Context context) throws IOException, InterruptedException {
            ...
            if (flag.equals("step3_2")) {// 同现矩阵item1：item2		n
...
//                System.out.println(k.toString() + "  " + v.toString());
            ...
            } else if (flag.equals("step3_1")) {// 评分矩阵item1		uid：pref
        ...
        //输出：评分矩阵:item	B:uid,pref
        	 //同现矩阵：item1	A：item2,n
    }
   ...
   //         System.out.println(key.toString() + ":");
            ...
            for (Text line : values) {
                ...
                if (val.startsWith("A:")) { //同现矩阵设置该key（item1）下item2对应的同现次数map
                    String[] kv = Recommend.DELIMITER.split(val.substring(2));
                    mapA.put(kv[0], kv[1]);

                } else if (val.startsWith("B:")) {	//评分矩阵设置该key（item）下所有uid对应的评分map
                    String[] kv = Recommend.DELIMITER.split(val.substring(2));
                    mapB.put(kv[0], kv[1]);
 ...
 //           System.out.println("SUMMA算法最终结果矩阵累加钱的一项矩阵：");            
            ...
            while (iter.hasNext()) {
                ...
                while (iterb.hasNext()) {
                    ...
                    context.write(k, v);
                    System.out.println("该用户："+k.toString() + "本项未累加结果为：" + v.toString());
           
                    
                    //其输出和前种方法map出的一样，为user（key）对item打分未累加向量的某一项
                }
            }
        }
    }
